...
namespace BoggleServer
{
    ...
    class BoggleGame
    {
        ...
        /// </summary>
        public bool playersConnected = true;
        ...
        /// <summary>
        /// Constructs a new boggle game with just one player.  Waits for second player but also keeps track if the 
        /// waiting player exits to alert the server that this game is no longer valid.
        /// </summary>
        public BoggleGame(Player one)
        {
            this.player1 = one;
        ...
        {
            timeRemaining = (int)startTime;

            // Send TIME messages every second.
            while (timeRemaining >= 0 && playersConnected)
            {
        ...
        private bool isDisconnected(string word, Exception e, bool isPlayer1)
        {
            if ( (word == null && e == null) || e != null)
            {
                ...
                //see which player disconnect send message to remaining player and close socket
                if (isPlayer1)
                {
        ...
        private void addToDatabase()
        {
            ...
            int pID1 = getPlayerID(player1.name);
            Console.WriteLine("Player 1 ID: " + pID1);

            int pID2 = getPlayerID(player2.name);
            Console.WriteLine("Player 2 ID: " + pID2);

            int gID = addToGameTable(pID1, pID2);
            Console.WriteLine("Game ID: " + gID);

            addToWordTable(gID, pID1, pID2);
        }
        ...
        private int getPlayerID(string name)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    command.CommandText = "SELECT * FROM cs3500_down.Player WHERE Name='" + name + "'";
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            {
                                return (int)reader["pID"];
                            }
                        }
                    }

                    MySqlCommand command1 = conn.CreateCommand();
                    command1.CommandText = "INSERT INTO cs3500_down.Player (Name) VALUES ('" + name + "'); " +
                                            "SELECT * FROM cs3500_down.Player WHERE Name='" + name + "'";

                    using (MySqlDataReader reader1 = command1.ExecuteReader())
                    {
                        if (reader1.Read())
                        {
                            return (int)reader1["pID"];
                        }
                    }
                }
                catch (Exception e)
        ...
        private int addToGameTable(int pID1, int pID2)
        {
            // Connect to the DB
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    // Create a command
                    MySqlCommand command = conn.CreateCommand();
                    command.CommandText = "INSERT INTO cs3500_down.Game (pID1, pID2, Board, TimeLimit, p1Score, P2Score, p1Name, p2Name) " + 
                        "VALUES (" + pID1 + ", " + pID2 + ", '" + board.ToString() + "', " + timeLimit + ", " + player1.score + ", " + player2.score + ", '"+ player1.name + "', '"+ player2.name+"'); " +
                        "SELECT MAX(gID) FROM cs3500_down.Game;";
        ...
        private void addWord(string word, int gID, int pID1, string legal)
        {
            // Connect to the DB
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    // Create a command
                    MySqlCommand command = conn.CreateCommand();
                    command.CommandText = "INSERT INTO cs3500_down.Word (WordPlayed, gID, pID, Legal) " +
                        "VALUES ('" + word + "', " + gID + ", " + pID1 + ", '" + legal + "');";
